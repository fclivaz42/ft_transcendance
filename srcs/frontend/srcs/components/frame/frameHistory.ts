...
async function createHistoryElement(match: MatchComplete): Promise<HTMLDivElement | null> {
...
async function loadHistory(matchElements: string[], page: number, elemPerPage: number): Promise<HTMLDivElement> {
	const template = document.createElement("template");
...
export default async function createHistoryFrame(): Promise<HTMLDivElement> {
	...
	const matches = await fetchHistory(RoutingHandler.searchParams.get("playerId") || undefined) as Array<MatchComplete>;
	const template = document.createElement("template");
...
async function updatePage(page: number, matches: MatchComplete[], matchElements: string[], elemPerPage: number) {
