...
export default function createLoadingFrame(message?: string): HTMLDivElement {
	...
	template.innerHTML = `
		<div class="flex items-center justify-center w-full h-full gap-8">
			...
			<h2 data-i18n="${sanitizer(message || "generic.loading")}" class="text-2xl font-semibold text-gray-700 dark:text-gray-300 mt-4">
				${sanitizer(message || i18nHandler.getValue("generic.loading"))}
			</h2>
		</div>
	`;
