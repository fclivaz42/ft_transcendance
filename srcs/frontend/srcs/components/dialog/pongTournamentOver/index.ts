...
import { GameOverPayload, TournamentOverPayload } from "../../../game/types";
...
async function createPongTournamentOverDialogContent(dialogRef: BackdropDialog, payload: TournamentOverPayload["payload"]): Promise<HTMLDivElement> {
	const winnerLabel = `
	...
	const users: Users[] = [await UserHandler.fetchUser(payload.)]
...
export function createPongTournamentOverDialog(payload: TournamentOverPayload["payload"]): HTMLDialogElement {
	const dialog = createDialog({ allowClose: false });

	const dialogContent = createPongTournamentOverDialogContent(dialog, payload);
